...
LANG_TEXTS = {
    "Turkce": {
        ...
        "quick": "1) Kaynak sec\n2) Islem kapsamini sec\n3) Onizleme\n4) Test kapatip uygula",
...
class FileGrouperApp(tk.Tk):
    ...
    def _configure_style(self) -> None:
        ...
        style.configure("Card.TFrame", background="#ffffff", bordercolor="#d9e2ef", relief="solid")
        style.configure("TLabel", background="#edf2f8", foreground="#243750")
        style.configure("Card.TLabel", background="#ffffff", foreground="#243750")
        style.configure("Header.TLabel", background="#18395a", foreground="#ffffff", font=("Helvetica", 17, "bold"))
        style.configure("SubHeader.TLabel", background="#18395a", foreground="#bfd2ea")
    ...
    def _build_ui(self) -> None:
        ...
        header.pack(fill="x")
        header.configure(padding=14)

        title_col = ttk.Frame(header, style="Card.TFrame")
        title_col.grid(row=0, column=0, sticky="w")
        self.title_label = ttk.Label(title_col, style="Header.TLabel", text="FileGrouper Python")
        self.title_label.pack(anchor="w")
        self.subtitle_label = ttk.Label(title_col, style="SubHeader.TLabel", text="")
        self.subtitle_label.pack(anchor="w", pady=(4, 0))

        controls_col = ttk.Frame(header, style="Card.TFrame")
        controls_col.grid(row=0, column=1, sticky="e")

        ttk.Label(controls_col, text="Language", style="Card.TLabel").grid(row=0, column=0, sticky="e", padx=(0, 6))
        lang_combo = ttk.Combobox(controls_col, values=["Turkce", "English"], textvariable=self.language_var, width=10, state="readonly")
        lang_combo.grid(row=0, column=1, sticky="e")
        lang_combo.bind("<<ComboboxSelected>>", lambda _: self._apply_language())

        self.dry_check = ttk.Checkbutton(controls_col, variable=self.dry_run_var)
        self.dry_check.grid(row=1, column=0, columnspan=2, sticky="e", pady=(6, 0))
        ...
        summary = ttk.Frame(right, style="Card.TFrame", padding=12)
        summary.grid(row=0, column=0, sticky="ew")
        self._build_summary_panel(summary)

        body = ttk.Frame(right, style="Card.TFrame", padding=8)
        body.grid(row=1, column=0, sticky="nsew", pady=(12, 0))
        body.rowconfigure(0, weight=1)
        body.columnconfigure(0, weight=1)
        self._build_tabs(body)

        footer = ttk.Frame(container, style="Card.TFrame", padding=10)
        footer.pack(fill="x", pady=(10, 0))
        ttk.Label(footer, textvariable=self.status_var, style="Card.TLabel").pack(side="left")
        self.progress_bar = ttk.Progressbar(footer, mode="determinate", variable=self.progress_var, maximum=100)
    ...
    def _build_left_panel(self, parent: ttk.Frame) -> None:
        ...
        self.source_btn = ttk.Button(source_row, command=self._browse_source)
        ...
        self.target_btn = ttk.Button(target_row, command=self._browse_target)
        ...
        self.mode_label = ttk.Label(parent, style="Card.TLabel")
        self.mode_label.pack(anchor="w")
        self.mode_combo = ttk.Combobox(parent, textvariable=self.mode_var, state="readonly")
        self.mode_combo.pack(fill="x", pady=(4, 8))

        self.dedupe_label = ttk.Label(parent, style="Card.TLabel")
        self.dedupe_label.pack(anchor="w")
        self.dedupe_combo = ttk.Combobox(parent, textvariable=self.dedupe_var, state="readonly")
        self.dedupe_combo.pack(fill="x", pady=(4, 8))

        self.similar_check = ttk.Checkbutton(parent, variable=self.similar_var)
        self.similar_check.pack(anchor="w", pady=(0, 10))

        filter_box = ttk.LabelFrame(parent)
        filter_box.pack(fill="x", pady=(0, 10))
        self.filters_frame = filter_box

        self.include_label = ttk.Label(filter_box)
        self.include_label.grid(row=0, column=0, sticky="w", padx=6, pady=(6, 2))
        ttk.Entry(filter_box, textvariable=self.include_ext_var).grid(row=1, column=0, sticky="ew", padx=6)

        self.exclude_label = ttk.Label(filter_box)
        self.exclude_label.grid(row=2, column=0, sticky="w", padx=6, pady=(6, 2))
        ttk.Entry(filter_box, textvariable=self.exclude_ext_var).grid(row=3, column=0, sticky="ew", padx=6)

        size_row = ttk.Frame(filter_box)
        size_row.grid(row=4, column=0, sticky="ew", padx=6, pady=(6, 2))
        ...
        date_row = ttk.Frame(filter_box)
        date_row.grid(row=5, column=0, sticky="ew", padx=6, pady=(6, 8))
        ...
        self.preview_btn = ttk.Button(actions, style="Primary.TButton", command=lambda: self._start_run(False))
        self.preview_btn.grid(row=0, column=0, sticky="ew", padx=(0, 4), pady=(0, 6))
        self.apply_btn = ttk.Button(actions, style="Primary.TButton", command=lambda: self._start_run(True))
        self.apply_btn.grid(row=0, column=1, sticky="ew", padx=(4, 0), pady=(0, 6))

        self.pause_btn = ttk.Button(actions, command=self._toggle_pause)
        self.pause_btn.grid(row=1, column=0, sticky="ew", padx=(0, 4), pady=(0, 6))
        self.cancel_btn = ttk.Button(actions, command=self._cancel_run)
        self.cancel_btn.grid(row=1, column=1, sticky="ew", padx=(4, 0), pady=(0, 6))

        self.undo_btn = ttk.Button(actions, command=self._undo_last)
        self.undo_btn.grid(row=2, column=0, sticky="ew", padx=(0, 4))
        self.export_btn = ttk.Button(actions, command=self._export_report)
        self.export_btn.grid(row=2, column=1, sticky="ew", padx=(4, 0))

    def _build_summary_panel(self, parent: ttk.Frame) -> None:
        self.progress_label = ttk.Label(parent, style="Card.TLabel", text="0%")
        ...
        for idx, key in enumerate(keys):
            ...
            label = ttk.Label(box, style="Card.TLabel")
            label.pack(anchor="w")
            value = ttk.Label(box, style="Card.TLabel", textvariable=self.summary_vars[key], font=("Helvetica", 14, "bold"))
            value.pack(anchor="w")
            self.summary_text_labels[key] = label
    ...
    def _build_tabs(self, parent: ttk.Frame) -> None:
        ...
        self.dup_list = tk.Listbox(dup_tab)
        self.dup_list.grid(row=0, column=0, sticky="nsew")

        log_tab.rowconfigure(0, weight=1)
        log_tab.columnconfigure(0, weight=1)
        self.log_text = tk.Text(log_tab, state="disabled", font=("Menlo", 11), bg="#f8fbff", fg="#2d3d54")
        self.log_text.grid(row=0, column=0, sticky="nsew")

        self.quick_label = ttk.Label(quick_tab, justify="left")
        self.quick_label.pack(anchor="nw", padx=12, pady=12)

    def _apply_language(self) -> None:
        text = LANG_TEXTS[self.language_var.get()]
        self.title(text["title"])
        self.title_label.config(text=text["title"])
        self.subtitle_label.config(text="Python GUI + CLI")
        ...
        self.pause_btn.config(text=text["pause"] if not self.paused else text["resume"])
    ...
    def _scope_enum(self) -> ExecutionScope:
        value = self.scope_var.get().lower()
        if "only" in value or "sadece grupla" in value:
            if "duplicate" in value or "kopya" in value:
                return ExecutionScope.DEDUPE_ONLY
            return ExecutionScope.GROUP_ONLY
        if "kopya temizle" in value and "grupla" not in value:
            return ExecutionScope.DEDUPE_ONLY
        return ExecutionScope.GROUP_AND_DEDUPE

    def _mode_enum(self) -> OrganizationMode:
        return OrganizationMode.MOVE if self.mode_var.get().lower() in {"move", "tasi"} else OrganizationMode.COPY

    def _dedupe_enum(self) -> DedupeMode:
        value = self.dedupe_var.get().lower()
    ...
    def _poll_queue(self) -> None:
        while True:
            ...
            if kind == "log":
                ...
                text = LANG_TEXTS[self.language_var.get()]
                self.status_var.set(text["status_cancelled"])
    ...
    def _handle_complete(self, result: RunResult) -> None:
        ...
        for group in result.duplicate_groups[:200]:
            keeper = group.files[0].full_path if group.files else "-"
            self.dup_list.insert(
                tk.END,
                f"{group.sha256_hash[:12]}..  x{max(0, len(group.files)-1)}  {format_size(group.size_bytes)}  {keeper}",
            )
        ...
        text = LANG_TEXTS[self.language_var.get()]
        self.status_var.set(text["status_done"])
        self._set_running(False)
